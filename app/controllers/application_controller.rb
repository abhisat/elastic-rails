...
class ApplicationController < ActionController::API
  ...
  def elastic_test
   ...
   # end
    ...
    $totalhits = Array.new
    $labels = Array.new
    $before = 1496361579786 #data['before']
    $after = 1496361579786 + 250000 #data['after']
    $interval = "2m" #data['interval'].scan(/\d/).join('')
    ...
    while j < $urltest.length do
      begin
        $mainresponse[j] = client.search q: $urltest[j] #data['urls'][j]
        $totalhits[j] = $mainresponse[j]['hits']['total']
        p("The response from " + $urltest[j] + ":")
        p($mainresponse[j])
      rescue Exception => x
